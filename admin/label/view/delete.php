...
try {
  ...
  $nama_label = $_GET['nama_label'];

  $sql = "DELETE FROM label WHERE id_label=?;";
  ...
  if ($stmt->rowCount() > 0) {
    ...
    setcookie("message_data", 'Berhasil menghapus label "' . $nama_label . '"', time() + 1, '/admin/label');
    header("Location: /admin/label");
  } else {
    setcookie("message_color", 'red', time() + 1, '/admin/label');
    setcookie("message_data", 'Gagal menghapus label "' . $nama_label . '"', time() + 1, '/admin/label');
    header("Location: /admin/label");
  }
